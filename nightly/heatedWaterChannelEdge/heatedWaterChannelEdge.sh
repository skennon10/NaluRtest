...
if [ -f $CWD/PASS ]; then
    ...
    determine_pass_fail $testTol "heatedWaterChannelEdge.log" "heatedWaterChannelEdge.norm" "heatedWaterChannelEdge.norm.gold"
    ...
    # run the second case
    mpiexec -np 4 ../../naluX -i heatedWaterChannelEdge_rst.i -o heatedWaterChannelEdge_rst.log
    determine_pass_fail $testTol "heatedWaterChannelEdge_rst.log" "heatedWaterChannelEdge_rst.norm" "heatedWaterChannelEdge_rst.norm.gold"
...
GlobalPerformanceTimeSecond=`grep "STKPERF: Total Time" heatedWaterChannelEdge_rst.log  | awk '{print $4}'`
totalPerfTime=`echo "$GlobalPerformanceTimeFirst + $GlobalPerformanceTimeSecond" | bc `
if [ $PASS_STATUS -ne 1 ]; then
    echo -e "..heatedWaterChannelEdge...... FAILED":" " $totalPerfTime " s" " max diff: " $GlobalMaxSolutionDiff
else
    echo -e "..heatedWaterChannelEdge...... PASSED":" " $totalPerfTime " s"
fi
